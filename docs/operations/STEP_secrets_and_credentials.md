...
- **`step.regiominer.com`** → the `step` web-app worker (custom domain added in
  the Cloudflare dashboard). The legal pages are folded into the web app
  (`apps/web-app/public/{privacy,terms,support}.html`, generated by
  `apps/regiominer-site/build.mjs` from `docs/legal/*.md`), so
  `https://step.regiominer.com/{privacy,terms,support}` serve them. These are the
  URLs in App Store Connect.
- `regiominer.moldovancsaba.workers.dev` → the standalone `regiominer` docs
  worker (same pages); kept as a fallback.

## Cloudflare token permission note

The `CLOUDFLARE_API_TOKEN` has account **Workers Scripts:Edit** (deploys workers
+ workers.dev and updates the `step` worker's assets, which is how the legal
pages went live on `step.regiominer.com`). It does **not** have the
`regiominer.com` zone's Workers-Routes/DNS:Edit, so binding new custom domains/
routes for that zone must be done in the dashboard (as `step.regiominer.com`
was).

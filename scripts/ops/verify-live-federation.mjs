...
const requiredRemote = process.env.STEP_REQUIRED_REMOTE_NODE ?? "chappie";
...
const remote = nodes.find((node) => node.name === requiredRemote);
if (!remote) fail(`required remote node not found: ${requiredRemote}`);
if (remote.reachable !== true || remote.inQuorum !== true) {
  fail(`${requiredRemote} is not reachable and in quorum`);
}
if (remote.transport !== "peer") fail(`${requiredRemote} is not peer-native`);
if (!String(remote.p2pAddress ?? remote.url ?? "").startsWith("/")) {
  fail(`${requiredRemote} does not expose a peer multiaddr`);
}
if (/^https?:\/\/(127\.0\.0\.1|localhost|0\.0\.0\.0)/i.test(String(remote.url ?? ""))) {
  fail(`${requiredRemote} is still exposed as localhost in fleet view`);
}
...
ok(`${requiredRemote} is peer-native and in quorum`);

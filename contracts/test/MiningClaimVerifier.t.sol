...
contract MiningClaimVerifierTest is StepFixture {
    ...
    function test_natural_claim_mints_reward_and_twin() public {
        ...
        MiningClaimVerifier.ValidatorSig[] memory sigs =
            _quorumSigs(claimHash, TRI_A, miner, 2);

        vm.expectEmit(true, true, false, true);
        emit MiningClaimVerifier.TriangleMined(TRI_A, miner, 0, BASE_REWARD, claimHash);
        verifier.finaliseNaturalClaim(claimHash, TRI_A, LEVEL, miner, CID, sigs);
    ...
    function test_replay_rejected_across_both_paths() public {
        ...
        _finaliseNatural(claimHash, TRI_A, miner);
    ...
    function test_insufficient_quorum_rejected() public {
        ...
        MiningClaimVerifier.ValidatorSig[] memory sigs =
            _quorumSigs(claimHash, TRI_A, miner, 1); // weight 50 < 100
        vm.expectRevert(
    ...
    function test_duplicate_or_unsorted_validators_rejected() public {
        ...
        sigs[0] = _vote(valKeys[0], claimHash, TRI_A, miner, true);
        sigs[1] = _vote(valKeys[0], claimHash, TRI_A, miner, true); // duplicate
    ...
    function test_signature_over_different_claim_rejected() public {
        ...
        sigs[0] = _vote(valKeys[0], keccak256("other"), TRI_A, miner, true);
        sigs[0].validator = valAddrs[0];
        sigs[1] = _vote(valKeys[1], claimHash, TRI_A, miner, true);
    ...
    function test_rejection_votes_do_not_count() public {
        ...
        sigs[0] = _vote(valKeys[0], claimHash, TRI_A, miner, false);
        sigs[1] = _vote(valKeys[1], claimHash, TRI_A, miner, false);
    ...
    function test_non_mineable_level_rejected() public {
        ...
        verifier.finaliseNaturalClaim(claimHash, TRI_A, 10, miner, CID, sigs);
    }
    ...
    function test_frozen_triangle_rejected() public {
        ...
        _finaliseNatural(keccak256("claim-after-unfreeze"), TRI_A, miner);
        assertEq(token.balanceOf(miner), BASE_REWARD);
    }

    function test_exhaustion_after_all_slots() public {
        for (uint32 s = 0; s < SLOTS; s++) {
            _finaliseNatural(keccak256(abi.encode("ex", s)), TRI_A, miner);
        }
        ...
        MiningClaimVerifier.ValidatorSig[] memory sigs = _quorumSigs(ch, TRI_A, miner, 2);
        vm.expectRevert(
            abi.encodeWithSelector(
        ...
        verifier.finaliseNaturalClaim(ch, TRI_A, LEVEL, miner, CID, sigs);

        // Other triangles unaffected.
        _finaliseNatural(keccak256("ex-other"), TRI_B, miner);
    }
